...
export const autoLogin = function() {
  Debug.info('autoLogin', 'Initializing autologin', {
    username: window.App.settings.vpnUsername
  });
  // we check if we have existing login and we auto login
  if (
    window.App &&
    window.App.settings.vpnUsername &&
...
export const downloadTarballAndExtract = function(url) {
  ...
  createReadStream({ url }, requestStream =>
...
export const downloadFileToLocation = function(url, name) {
...
var createReadStream = (requestOptions, callback) =>
  callback(request.get(requestOptions));

// move file
export const copyToLocation = function(targetFilename, fromDirectory) {
...
export const rmdirSync = function(dir) {
...
export const copy = function(source, target, cb) {
