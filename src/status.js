    ...
    if (window.pendingCallback) {
      if (data) {
        ...
        if (window.connectionTimeout && type === 'c' && !window.connected) {
          ...
          return OpenVPNManagement.getLog((err, log) => {
            ...
            return window.App.VPN.disconnect().then(() =>
        ...
        if (type === 'd' && data.connected === false) {
...
export const disconnectUser = () =>
  ...
  setTimeout(() => {
    ...
    if (window.connectionTimeoutTimer) {
      return clearTimeout(window.connectionTimeoutTimer);
    }
  }, 5000);

export const monitorStatus = type => {
  if (window.timerMonitor) {
    clearTimeout(window.timerMonitor);
  }
