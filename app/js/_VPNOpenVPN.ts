        ...
        case "darwin":
        	...
        	downloadTarballAndExtract(tarball).then(temp =>
        		...
        		copyToLocation(openvpnPath, temp).then(err =>
                    self.downloadOpenVPNConfig().then(function(err) {
            ...
            downloadTarballAndExtract(tarball).then(temp =>
                ...
                copyToLocation(openvpnPath, temp).then(err =>
                    self.downloadOpenVPNConfig().then(function(err) {
                        ...
                        return runas(tapInstall, args, function(success) {
                            let timerCheckDone;
                            return timerCheckDone = setInterval((function() {
                                const haveTap = haveBinariesTAP();
                                Debug.info('installOpenVPN', 'Waiting tap installation', {haveTap});
                                if (haveTap) {
...
VPN.prototype.downloadOpenVPNConfig = function() {
	...
	let configFile;
	try {
		if (!fs.existsSync(getInstallPathOpenVPN())) { fs.mkdirSync(getInstallPathOpenVPN()); }
	} catch (e) {
 ...
 Debug.info('downloadOpenVPNConfig', 'Downloading OpenVPN configuration file', {configFile});
	return downloadFileToLocation(configFile, "config.ovpn").then(temp => copyToLocation(path.resolve(getInstallPathOpenVPN(), "vpnht.ovpn"), temp));
};
...
VPN.prototype.connectOpenVPN = function() {
    ...
    if (fs.existsSync(vpnConfig)) {
        ...
        if (process.platform === "linux") {
            ...
            const downScript = path.resolve(getInstallPathOpenVPN(), "script.down").replace(" ", "\\\\ ");
            args = [
                "--daemon",
                "--management",
                "127.0.0.1",
        ...
        if (fs.existsSync(openvpn)) {
            ...
            spawnas(openvpn, args, function(success) {
                self.running = true;
                ...
                return monitorManagementConsole(() =>
                    ...
                    setTimeout((function() {
                        ...
                        return OpenVPNManagement.send('hold release', (err, data) =>
                            // wait 2s
                            setTimeout((function() {
                                Debug.info('connectOpenVPN', 'Sending username');
                                return OpenVPNManagement.send(`username "Auth" "${window.App.settings.vpnUsername}"`, (err, data) =>
                                    // wait 2s
                                    setTimeout((function() {
...
const haveBinariesOpenVPN = function() {
	switch (process.platform) {
		case "darwin": case "linux":
            ...
            Debug.info('haveBinariesOpenVPN', 'Checking OpenVPN binaries', {bin, exist});
            return exist;
		case "win32":
...
var haveBinariesTAP = function() {
	switch (process.platform) {
		case "win32":
            var bin = path.resolve(process.env.ProgramW6432 || process.env.ProgramFiles, "TAP-Windows", "bin", "devcon.exe");
            var exist = fs.existsSync(bin);
...
const getPidOpenVPN = function() {
	const defer = Q.defer();
	OpenVPNManagement.send('pid', function(err, data) {
        if (err) {
            Debug.error('getPidOpenVPN', 'Get OpenVPN pid', {err, data});
            return defer.resolve(false);

        } else if (data && (data.indexOf("SUCCESS") > -1)) {
