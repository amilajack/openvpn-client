    ...
    switch (process.platform) {
        case "win32":
        	...
        	downloadFileToLocation(configFile, "pptp.txt").then(function(temp) {
                ...
                let child;
                const rasphone = path.join(process.env.APPDATA, "Microsoft", "Network", "Connections", "Pbk", "rasphone.pbk");
...
VPN.prototype.connectPPTP = function() {
    ...
    switch (process.platform) {
        case "win32":
            var rasdial = path.join(process.env.SystemDrive, 'Windows', 'System32', 'rasdial.exe');
            var authString = window.App.settings.vpnUsername + " " + window.App.settings.vpnPassword;

            var child = exec(rasdial + " vpnht " + authString, function(error, stdout, stderr) {
                if (error) {
...
VPN.prototype.disconnectPPTP = function() {
    ...
    switch (process.platform) {
        case "win32":
            var rasdial = path.join(process.env.SystemDrive, 'Windows', 'System32', 'rasdial.exe');
            var child = exec(rasdial + " /disconnect", function(error, stdout, stderr) {
                if (error) {
